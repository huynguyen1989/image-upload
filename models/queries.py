queries_templates = {
    'sql_select_categories' : """SELECT `CategoryID`, `CategoryName`, `Description`, `IconURL` FROM Categories c ORDER BY c.`Ordering`""",
    'sql_select_category_by_id' : """SELECT `CategoryID`, `CategoryName`, `Description`, `Ordering`, `IconURL` FROM Categories c WHERE c.CategoryID=%s""",
    'sql_create_category': """ INSERT INTO `Categories` (`CategoryName`, `Description`, `Ordering`, `IconURL`) VALUES (%s, %s, %s, %s) """,
    'sql_delete_cateory': """DELETE FROM `Categories` WHERE CategoryID=%s""",
    'sql_update_category': """UPDATE Categories SET CategoryName=%s, Description=%s, Ordering=%s, IconURL=%s WHERE CategoryID=%s""",
